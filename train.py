...
def train(args, device, loss_net, image_transform_net, train_loader):
    # Optimizer
    optimizer = optim.Adam(ImageTransformerNet.parameters(), lr = args.lr)
    # Loss Function
    ...
    # Get Style Features - ys
    B, C, H, W = style_tensor.shape
    style_features = LossNet(style_tensor.expand([args.batch_size, C, H, W]))
    style_gram = {}
    ...
    for epoch in range(args.epochs):
        for content_batch, _ in train_loader:
            ...
            generated_batch = image_transform_net(content_batch)
    ...
    # Save ImageTransformNetwork weights
    image_transform_net.eval()
    image_transform_net.cpu()
...
def main():
    ...
    loss_net = LossNet.lossNet_pretrained().to(device)
    image_transform_net = transform_net.ImageTransformNet().to(device)
    train(args, device, loss_net, image_transform_net, train_loader)
