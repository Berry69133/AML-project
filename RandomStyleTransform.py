...
class RandomStyleTransform:
    ...
    def up_sample(self, dataset: FER2013Dataset, weights: torch.tensor):
        device = "cuda" if torch.cuda.is_available() else "cpu"
        ...
        for label in range(7):
            ...
            for _ in range(0, labels_to_generate[label], 100):
                ...
                dataset_aug.append_images(samples)
                dataset_aug.append_labels(torch.ones(samples.size(0)) * label)
